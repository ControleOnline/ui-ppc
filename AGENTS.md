...
- Em `displayType=tv`, a tela principal deve manter o mapa de entregas e sobrepor um dock de cards operacionais no canto inferior direito. Esse dock alterna os cards a cada 30s, consome `/report/orders/operational-insights`, nao mostra informacao financeira e nao deve depender do renderer de pedidos.
- Cada card operacional da TV deve buscar apenas o seu proprio bloco quando ficar visivel, usando `insight=<chave>` no endpoint de reports; nada de pré-carregar o summary inteiro no container da tela.
- Em `tv`, nao desenhar traços laterais de hierarquia, badge de fila redundante ou legenda de status que ja esteja no topo da tela. O monitor precisa ficar como mapa + cards operacionais, nao como lista operacional de pedidos.
